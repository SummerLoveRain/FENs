...
class PINNConfig(BaseConfig):
    ...
    def optimize_one_epoch(self):
        ...
        with torch.no_grad():        
            ...
            def levenberg_marquardt(f, jac, x0, tol=1e-6, lambda_init=1e-3, div_factor=1.3, mul_factor=3, lambda_update=1, max_iter=50000):
                x = x0
                lbd = lambda_init
                for iter in range(max_iter):
                    r = f(x)
                    J = jac(x)
                    JT = torch.transpose(J, dim0=0, dim1=1)
                    I = torch.eye(JT.shape[0], dtype=float).to(self.device)
                    JTJ = torch.matmul(JT, J) + lbd*I
                    JTr = torch.matmul(JT, r)
                    delta_x = torch.linalg.lstsq(JTJ, -JTr)[0]
                    # delta_x = torch.linalg.solve(JTJ, -JTr)
                    # delta_x = torch.linalg.pinv(JTJ, -JTr)
                    x_new = x + delta_x
                    
                    if iter % lambda_update == 0:
                        # 检查目标函数值是否下降
                        r_new = f(x_new)
                        if torch.linalg.norm(r_new) < torch.linalg.norm(r):
                            # 接受更新
                            x = x_new
                            lbd = max(lbd/div_factor, 10**(-9))  # 减小阻尼因子
                        else:
                            # 拒绝更新，增加阻尼因子
                            lbd = min(mul_factor*lbd, 10**(8))
                    
                    loss = 1/2*torch.sum(r**2)
                    loss = np.max(self.detach(loss))
                    print('iter=' + str(iter+1) + ' loss=' + str(loss))
            ...
            # Newton迭代
            W = levenberg_marquardt(loss_f, jac_f, Wk)           
