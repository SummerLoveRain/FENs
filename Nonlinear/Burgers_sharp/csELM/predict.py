...
if __name__ == "__main__":
    ...
    for TIME_STR in TIME_STRs:
        ...
        scipy.io.savemat(root_path + '/' +
                        TASK_NAME + '/' + TIME_STR + '/data1.mat', {'U_pred': U_pred, 'U_star': U_star, 'X': X_TRAIN, 'T': Y_TRAIN, 'E': U_pred-U_star})
        
        file_name = root_path + '/' + TASK_NAME + '/' + TIME_STR + '/Burgers_heatmap3'
        # plot_heatmap3(X, Y, T, P, E=None, xlabel=None, ylabel=None, T_title=None, P_title=None, E_title=None, file_name=None, abs=True):
        plot_heatmap3(X=X_TRAIN, Y=Y_TRAIN, T=U_star, P=np.abs(U_pred), E=None, xlabel='x',
                    ylabel='t', file_name=file_name)
        # plot_heatmap3(X=X_TRAIN, Y=Y_TRAIN, T=U_star, P=U_pred, E=None, xlabel='x',
        #             ylabel='t', file_name=file_name, vmin=np.min(U_star), vmax=np.max(U_star), vmaxe=np.max(errors))
