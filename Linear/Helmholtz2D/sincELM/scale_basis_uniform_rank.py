...
print(f"Working directory is now: {os.getcwd()}")
current_dir = os.path.dirname('/home/lunxu/yangqh/FEN/Linear/Helmholtz2D/sincELM/')
set_dir(current_dir)
...
rank_400 = ranks[:top_steps]
rank_900 = ranks[top_steps:2*top_steps]
rank_1600 = ranks[2*top_steps:3*top_steps]
rank_2500 = ranks[3*top_steps:4*top_steps]

datas2 = []
data = np.stack((scales, rank_400), 1)
datas2.append(data)
data = np.stack((scales, rank_900), 1)
datas2.append(data)
data = np.stack((scales, rank_1600), 1)
datas2.append(data)
data = np.stack((scales, rank_2500), 1)
datas2.append(data)

data_labels2 =[]
data_labels2.append('rank of M='+str(Ns[0]))
data_labels2.append('rank of M='+str(Ns[1]))
data_labels2.append('rank of M='+str(Ns[2]))
data_labels2.append('rank of M='+str(Ns[3]))

file_name2 = root_path + '/' + path + '/u_infty_scale_rank'
xy_labels = ['scale', 'error', 'rank']
from plot.line import plot_line2
plot_line2(datas,
              data_labels,
              datas2,
              data_labels2,
...
file_name2 = root_path + '/' + path + '/loss_infty_scale_rank'
xy_labels = ['scale', 'error', 'rank']
from plot.line import plot_line2
plot_line2(datas,
              data_labels,
...
def set_dir(path):
    os.chdir(path)
    log(f"Working directory is now: {os.getcwd()}")
    
log(f"Working directory is now: {os.getcwd()}")
current_dir = os.path.dirname('/home/dell/yangqh/FEN/Linear/Poisson1D/sincELM/')
set_dir(current_dir)
