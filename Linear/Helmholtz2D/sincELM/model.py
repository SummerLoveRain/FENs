...
def get_model(model_dict, model_name):
    ...
    elif model_name == MODEL_NAME.sincELM:
        model = sincELM(**model_dict)
    else:
...
class sinELM(Base_Model):
    ...
    def diff_x(self, x, idx, shift_coeff=1):
        ...
        trans_omega_idx = trans_omega[idx:idx+1, :]
        diff_us = trans_omega_idx * us
        return diff_us
...
class sincELM(Base_Model):
    ...
    def diff_x(self, x, idx, shift_coeff=1):
        ...
        trans_omega_idx = trans_omega[idx:idx+1, :] * shift_coeff
        diff_us = (pi * trans_omega_idx * y * torch.cos(y) - pi * trans_omega_idx * torch.sin(y))/(y**2+epsilon)
        return diff_us
    ...
    def diff_xx(self, x, idx, shift_coeff=1):
        ...
        trans_omega_idx = trans_omega[idx:idx+1, :] * shift_coeff
        diff_us = (2*trans_omega_idx**2*pi**2*y*torch.sin(y) - 2*trans_omega_idx**2*pi**2*y**2*torch.cos(y)-trans_omega_idx**2*pi**2*y**3*torch.sin(y))/(y**4+epsilon)
        return diff_us
