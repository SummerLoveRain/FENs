...
class PINNConfig(BaseConfig):
    def __init__(self, param_dict, train_dict, model):
        ...
        # 加载训练参数
        x, y, t, xb, yb, tb, x0, y0, t0, x_valid, y_valid, t_valid, self.funF, self.funU, self.N_basis = self.unzip_train_dict(
    ...
    def optimize_one_epoch(self):
        ...
        with torch.no_grad():
            ...
            # all_us = torch.cat((us, ubs, u0s), dim=0)
            # rank = np.max(self.detach(torch.linalg.matrix_rank(all_us)))
