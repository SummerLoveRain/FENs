...
def get_model(model_dict, model_name):
    ...
    elif model_name == MODEL_NAME.FNN:
        model = FNN(**model_dict)
    else:
...
class SLNN(Base_Model):
    def __init__(self, in_num, out_num, act_func=nn.Tanh(), scale=1, init_type=INIT_TYPE.Xavier_Normal, init_params={}):
        super().__init__(init_type, init_params)
        self.in_num = in_num
        self.out_num = out_num      
        self.N_basis_F = self.out_num
...
class FNN(Base_Model):
    def __init__(self, layers, act_func=nn.Tanh(), init_type=INIT_TYPE.Xavier_Normal, init_params={}):
        ...
        self.linear_list = []
        for i in range(len(self.layers)-2):
            linear = SLNN(self.layers[i],self.layers[i+1], act_func=act_func, init_type=init_type, init_params=init_params)
            self.linear_list.append(linear)
        self.linear_list = nn.ModuleList(self.linear_list)
        linear = SLNN(self.layers[i],self.layers[i+1], act_func=act_func, init_type=init_type, init_params=init_params)
        self.fc = linear
